...
export default function App() {
  ...
  const [toastNotif, setToastNotif] =
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (authUser) => {
      ...
      try {
        ...
        if (!profile) {
          throw new Error(
            'User profile could not be loaded or created.'
          );
        }
        ...
        if (
          ...
          email === 'admin@assignme.in'
        ) {
          profile.role = 'admin';
        }

        setUserProfile(profile);
        setPortal(profile.role === 'admin' ? 'admin' : 'user');
      } catch (error) {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(
      ...
      (snapshot) => {
        ...
        setNotifs((previousNotifications) => {
          const newest = list[0];

          if (
            newest &&
            newest.id !== previousNotifications[0]?.id &&
            !newest.read
          ) {
  ...
  useEffect(() => {
    const checkRoute = () => {
      ...
      const isAdminAuth = userProfile?.role === 'admin';
  ...
  const handleOrderSuccess = () => {
    alert(
      "🎉 Order placed successfully! Check your WhatsApp window to finalize. Our team is writing currently."
    );
  };
    ...
    <div className="min-h-screen bg-[#020b18] text-[#e8f0fe] flex flex-col justify-between font-sans selection:bg-[#1a6fff]/30">
      ...
      <footer className="border-t border-[#0d2d50] bg-[#040f1e] py-10 px-6 md:px-14 lg:px-20 text-center text-[#7da3cc] text-xs">
        ...
        <div className="border-t border-[#0d2d50]/40 pt-6 max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4 text-[11px]">
          <span>
            © 2025 Assign Me. Designed with absolute precision. All rights
            reserved.
          </span>

          <span className="opacity-60">
            Because assignments shouldn't stress you.
          </span>
        </div>
      </footer>
